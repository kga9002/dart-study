...
void main() async {
  ...
  String filename = stdin.readLineSync(encoding: utf8)!;

  stdout.write("저장할 파일 내용을 입력하세요: ");
  String content = stdin.readLineSync(encoding: utf8)!;

  Directory("/quiz").createSync(recursive: true);
  // 파일 객체 생성
  var file = File("quiz/$filename.txt");
  ...
  // 파일 다시 읽어서 출력
  String result = await readFileToString("quiz/$filename.txt");
  print(result);
}
